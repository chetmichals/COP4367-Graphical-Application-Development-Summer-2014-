...
function onMouseDownEventHandler(e) {
    switch(drawMode) {
        ...
        case "SprayStyle":
            ...
            for (var i = 0; i < 5; i++) {
                g.beginPath();
                newX += (Math.random() * 75) - (Math.random() * 75);
                newY += (Math.random() * 75) - (Math.random() * 75);
...
{
    var tempX = (Math.random() * 100) - (Math.random() * 100)
    var tempY = (Math.random() * 100) - (Math.random() * 100);
    g.moveTo(x+0,y+0);
    g.lineTo(x+tempX,y+0);
    g.lineTo(x+0,y+tempY);
    g.fill();
}
